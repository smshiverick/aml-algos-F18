...
function of the predictor variables \cite{james13, raschka17}. The decision 
tree and random forests models are preferred for modeling the predictors as 
non-linear functions. The main advantage of logistic regression is that 
the coefficient estimates can be interpreted in terms of odds ratios. 
With pre-pruning, decision trees provide an interpretable model, but are
prone to overfit data used to train the model. The random forests model
corrects for overfitting in single decision trees and generalizes well 
to new data, but can be difficult to interpret. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The top three model all identified cocaine use as the most informative feature 
for predicting pain reliever misuse and abuse (PRLMISAB). The models differed, however, in the importance that each assigned to the remaining predictor 
variables. For a one unit increase in cocaine use, the odds of PRLMISAB 
increased by 1.99 times (holding the effects of other variables constant). 
The proportion of PRLMISAB among individuals who reported moderate to high
cocaine use (39\%) was more than five times larger than for individuals who 
reported no or low cocaine use (7\%), in the decision tree model.


 


amphetamines, tranquilizers, heroin use, age category, and mental health as 
important features for predicting .
The models differed 

All three models selected cocaine  PRLMISAB.  The random forests model and logistic regression both 
