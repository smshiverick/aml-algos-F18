...
that the proportion respondents who had not misused or abused prescription 
opioid pain reliever was much larger than the 11\% who reported that they had. 
Logistic regression, random forests, and decision trees performed better than 
other models for classifying pain reliever misuse and abuse using the 
$f_1$-score as the appropriate performance metric. Traditional algorithms 
tend to classify new observations based on the majority class, as seen with 
the naive Bayes model, which had a relatively high $f_1$-score despite only 
35 true positive instances and a high number of false negatives. The random 
forests model performed slightly better than the other models as it correctly 
predicted more true positives instances than the logistic regression and 
had fewer false negative errors than the single decision tree. The QDA and LDA 
...
outcome are non-linear. The terms in the logistic regression equation can be 
rearranged to obtain the function of the event probability (equation 3). 
...
Feature importance for the random 
...
Random forests correct for overfitting by 
