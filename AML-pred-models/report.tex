...
The present study uses a supervised 
learning approach to classify instances of pain reliever misuse and abuse 
using several classification models. 
...
Precision, and the $f_1-score$. In terms of model accuracy, the multilayer 
perceptron neural networks, support vector classifier, random forests, and 
logistic regression were among the top performers. However, as described in 
the interaction, in cases of unbalanced classes, when with greatly more 
instances of one class than another class | and given that the proportion of 
respondents who had never misused or abused pain relievers was much greater 
than the proportion who had| then the $f_1$-score is a better metric of model 
performance than accuracy because it takes into account both sensitivity and 
precision. The classification models with the highest $f_1$-score were 
logistic regression, decision trees, and random forests, followed by linear 
discriminant analysis (LDA) and the naive Bayes classifier. 
