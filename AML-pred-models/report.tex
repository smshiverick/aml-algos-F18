...
synthetic opioids such as fentanyl \cite{jones15}. Because the dosage levels 
and potency of illicit and synthetic opioids are largely unknown, The risk 
of overdose death has increased. The sharp rise in prescription 
overdose deaths (POD) and heroin overdose deaths (HOD) are positively 
correlated \cite{muhuri13, unick13}. Predictive modeling approaches can 
identify individuals susceptible for opioid addiction and may provide 
insights to inform policy decisions for addressing the opioid crisis. 
...
comprehensive public survey that includes more than 2600 variables on a 
...
also with new observations that are previously `unseen'. A standard practice 
is to divide the sample dataset into two portions: a \emph{training set} and 
a \emph{testing set} of observations that is set aside and used to evaluate 
model performance. By convention, approximately 70 to 80 percent of 
...
As stated by George Box, "All models are wrong, but some models are useful"\cite{box05}. There are advantages and limitations with any 
classification model, but one of the most useful and interpretable models 
is logistic regression. Logistic regression models the conditional 
...
greater than fifty-percent, $Pr(PRLMISAB) > 0.5$. Logistic regression 
...
not. The distribution of probabilities in the logit model has an S-shaped 
curve. The coefficients estimates obtained from a logistic regression model, 
...
The \emph{Support Vector Classifier} (SVM) extends the maximal margin classifier 
by using a soft margin that allows a small number of observations to be 
misclassified on the wrong side of hyperplane. The observations that fall 
directly on the margin or on the wrong side of the hyperplane are called `support 
vectors'. The parameter `C' indicates the number of observations that can 
violate the margin; if $C>0$, not more than C observations can be on the wrong 
side of the hyperplane. SVC addresses the problem of non-linear boundaries 
between classes by enlarging the feature space with higher order (e.g., quadratic, 
cubic, polynomial) functions of the predictors. Support vector machines (SVM) 
is an extension of SVC that uses a kernel. One of the most commonly used 
approaches is the radial basis function (RBF) kernel is  (i.e., Guassian kernel).
In training the model, only a subset of data points is used to construct 
the decision boundary, the support vectors that lie on the border separating
...
decision boundary obtained with SVM using the RBF kernel; the decision
boundary is a smooth curve and the support vectors are large points with 
bold outlines. 

A high value of C stresses the importance of each individual sample and the 
SVC model will try to fit the training set as best as possible, which increases 
the risk of overfitting. With a low value of C the classification algorithm 
adjusts to the majority of data points, and the influence of each data point 
on the decision boundary is limited.


Low values of C correspond to strong regularization,  this leads to many support vectors, so many alphas are non-zero
...
\begin{figure}[!ht]
  ...
  \caption{Decision Boundary and Support Vectors for SVM with RBF kernel 
  \cite{muller17}}
  \label{f:Figure4}
\end{figure}
...
\begin{figure}[!ht]
  ...
  \caption{Residual and Predicted Values for Logistic Regression of 
  Pain Reliever Misuse and Abuse fit to the Testing Set (n=}
  \label{f:Figure2}
\end{figure}
...
number of features (100 or more) or with sparse datasets with many values equal 
to zero.
