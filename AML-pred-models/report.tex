...
matrices reported in Table 3, which includes the performance metrics of 
accuracy, sensitivity (i.e., Recall), precision, and the $f_1$-score. 
The neural networks (MLP) and support vector classifier had the highest model 
accuracy; however, as described in the introduction, the $f_1$-score 
is a better performance metric than accuracy with datasets that have 
unbalanced classes because it takes into account both sensitivity and precision. 
In the present data set, there is a clear indication that the classes were
unbalanced, given that the proportion of respondents who had never misused 
or abused pain relievers in the NSDUH dataset was much greater than the 
proportion who had, approximately 11\% of respondents in both the training 
and test sets reported misusing and abusing opioid pain relievers. Therefore, 
using the $f_1$-score as the preferred performance metric, logistic regression, decision trees, and random forests, had the best performance given that they
were tied for the highest $f_1$-score.

, with 
| instances of one class are much greater than 
instances of the other class | Given that followed by linear discriminant analysis (LDA) and the 
