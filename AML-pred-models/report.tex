...
are interpreted as the change in the log of the odds of the positive outcome
of the dependent variable, $Y_i=True$, occurring given a one unit change in each 
$X_i$...$X_j$ variable, holding constant the effects of other independent
variables. The parameters relate to the log of the odds ratio rather than to 
the dependent variable directly. The odds are calculated as the probability of 
the event occurring divided by the probability of the event not occurring. 


The odds ratios are obtained 
by taking the antilog of estimated coefficient, which is the exponentiated 
parameter estimates: $e^x$. For an interval scaled independent variable, the 
odds ratio  is interpreted as the multiplicative increase in the odds of the 
dependent variable event happening (e.g., PRLMISAB) given a one unit change in 
the value of the independent variable, assuming the effects of all other 
independent variables are held constant. For a dichotomous (dummy) independent 
variable, e.g., sex, the multiplicative increase in the odds of the dependent 
variable event happening if the event represented by the independent 
variable occurs, assuming the effects of all other independent variables 
are constant \cite{gujarati09}.  
...
NOTE: The odds ratio does not reveal the probability of a student obtaining 
a private sector job.  it only reveals the ratio of success to failure; to get 
the probability for a particular student, we need to look at y-hat and p-hat.
