...
The multilayer perceptron (MLP) had the highest accuracy (90.6\%); however, 
the $f_1$-score was 0.88, which indicates that it was not a good model for 
data with imbalanced classes \cite{yun09}. Figure 5 shows the MLP model which 
takes the features as the input to the model, a single hidden-layer is 
comprised of the weighted combination of the input variables, and the output 
layer represents a response probability. During model training, the weights 
of the predictor variables are first randomly initialised and then iteratively 
adjusted so as to minimize an error function (e.g., gradient descent)
\cite{brown12}. With a limited number of predictor variables and single 
hidden layer, the neural network is still somewhat interpretable. 
...
The results showed imbalanced classes in the sample data in that the 
...
Using the $f_1$-score as the preferred evaluation metric, logistic 
regression, decision trees, and random forests performed best among the
models for classifying pain reliever misuse and abuse. 


% >>> Talk about performance metrics <<<
