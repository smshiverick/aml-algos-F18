...
may help predict individuals at risk for opioid addiction. The present study 
compares ten linear classification models . The sample data consisted of 
N = 114,038 respondents from the National Survey on Drug Use and Health 
(NSDUH) for 2015 and 2016. Of the total sample, 27\% had used any opioid 
pain reliever medication in the past year, and 11\% of respondents had 
previously misuse or abused pain relievers; 2\% of the sample reported 
heroin use. The classifier models were fit to a training set with fifteen
predictor variables and pain reliever misuse and abuse as the target outcome. 
Class labels were predicted for the testing set and model performance was 
evaluated in a confusion matrix. Four performance metrics were considered: 
accuracy, sensitivity (recall), precision, and $f_1$-score. Neural networks 
(MLP) and support vector classifier had the highest model accuracy; however, 
logistic regression, decision trees, and random forests, had the highest 
$f_1$-score. The $f_1$-score is considered a better metric of performance 
than accuracy With data that has unbalanced unbalanced classes. Advantages 
and limitation of the classification models are discussed. 
...
Performance of the classification models was evaluated in the confusion matrices
reported in Table 3, which includes the performance metrics of Accuracy, 
Sensitivity (i.e., Recall), Precision, and the $f_1$-score. The neural network
(MLP), support vector classifier, random forests, and logistic regression had
the highest model accuracy; however, as described in the introduction, with 
data that has unbalanced classes | instances of one class are much greater than 
instances of the other class | the $f_1$-score provides a better metric of model 
performance than accuracy because it takes into account both sensitivity and 
precision. Given that the proportion of respondents who had never misused or 
abused pain relievers in the NSDUH dataset was much greater than the proportion 
who had, logistic regression, decision trees, and random forests, had the 
highest $f_1$-score, followed by linear discriminant analysis (LDA) and the 
...
\begin{table*}[ht]
  ...
  \begin{tabular}{llllllll}
    ...
    Random Forests & & No Misuse & PRL Misuse &  &  &  & \\
     ...
     & PRL Misuse & 320 & 665 &  &  &  & \\
    \midrule
    Boosted Trees & & No Misuse & PRL Misuse &  &  &  & \\
...
sampled data points and the maximum number of features; the algorithm could 
...
very similar and will fit the data easily based on the most distinctive 
features, whereas a low value will produce trees that are very different 
from each other, and reduces over-fitting. 


Random forests is an ensemble approach that builds many trees and averages 
their results to reduce overfitting. 

The parameters of interest for building random forests are: (a) the number of 
trees (n-estimators), (b) the number of data points for bootstrap sampling 
...
The model was build on the training set and evaluated on the test set, with
both training set and test set accuracy equal to 0.984. To reduce overfitting, 
pre-pruning could be implemented by reducing the maximum depth, or by reducing 
the learning rate. 
...
The model accuracy for gradient boosting was no better than random forests, 
and this is because the default parameter settings were used; further parameter 
tuning is needed to improve model performance.
...
Figure 7 shows that the feature importance 
