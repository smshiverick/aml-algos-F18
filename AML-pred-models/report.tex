...
each day, The number of opioid-related overdose deaths has more than 
quadrupled from 1999 to 2016. \cite{cdc18, Rudd16}. Nonmedical use of 
...
0 \cite{gujarati09}. The logistic or logit function is represented below,
where X = ($X_1$...$X_p$) predictors: 
$ Pr(abuse=1) = F(\alpha+\sum_j(\beta_i*X_ij)) $
...
and identified factors that influence MUPO. The goal of the present study is 
to compare the performance of different classifier models of pain reliever 
misuse and abuse and evaluate each model using common performance metrics. 

When instances of one class in a dataset 
greatly outnumber instances of the other class, traditional classification 
algorithms tend to classify observations as belonging to the majority class, 
although frequently it is the case that the class of interest (positive class)
...
data (e.g., `boosting'). As described above, with imbalanced classes the 
$f_1$-score is a better metric than than accuracy, as it takes into account 
both sensitivity (i.e., recall) and precision. The $f_1$-score was used as the 
preferred metric in evaluating model performance. A second goal was to identify 
