...
(i.e., Recall), precision, and the $f_1$-score. Because there was a clear 
indication of unbalanced classes in the sample data | the proportion of 
respondents who had never misused or abused pain relievers (89\%) was 
much greater than the proportion who had | the $f_1$-score was used as
the preferred metric of performance rather than accuracy because it takes 
into account both sensitivity and precision. Although the neural networks 
(MLP) model and support vector classifier (SLC) had the highest accuracy, 
they both had very low $f_1$-scores compared to other models. In terms 
of $f_1$-score logistic regression, decision trees, and random forests 
performed better than the other classifiers and were tied for the highest 
score. Examination of the confusion matrices reveals that the random forests 
model identified more true positives than the decision trees and had fewer 
misclassification errors than logistic regression. The LDA model
performed similarly to logistic regression in terms of $f_1$-score and
accuracy, more true positives and fewer false positives, but more
false negatives. It is interesting to note that the QDA model had the 
highest number of true positives, but a lower $f_1$-score than the naive 
Bayes classifier, even though the naive Bayes model returned only 30 true 
positives. The main parameter settings for the classification models are 
presented in Table 4.

\subsubsection{Linear Models}

Together, all of the features entered into the logistic regression model
significantly predicted pain reliever misuse and abuse. The parameter estimates 
for the logistic regression classifier model are presented in Table 5, sorted 
in descending order by z-score. The estimates for all of the features were 
statistically significant ($p<.001$), except for mental health treatment 
which was non-significant. The model was rerun leaving out mental health 
treatment, and excluding this variable this did not significantly change 
...
age category, and heroin use were identified as six of the most influential predictors of pain reliever misuse and abuse. 
